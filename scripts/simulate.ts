...
type Style = "random" | "cruel" | "kind" | "obedient";
function play(style: Style): RunState {
  ...
  for (const sc of scenesFor(s.cond)) {
    ...
    const score = (c: any) => style === "cruel" ? (c.fx?.harm || 0) * 2 + (c.fx?.dehumanize || 0) : style === "kind" ? (c.fx?.mercy || 0) * 2 + (c.fx?.defiance || 0) : style === "obedient" ? (c.fx?.obedience || 0) * 2 - (c.fx?.defiance || 0) : Math.random();
    if (sc.kind === "narrative") rec("seen");
    else if (sc.kind === "rate") rec(style === "kind" ? 9 : style === "cruel" ? 2 : Math.round(Math.random() * 10));
    else if (sc.kind === "slider") { const v = style === "cruel" ? sc.max : style === "kind" ? sc.min : Math.round(Math.random() * sc.max / sc.step) * sc.step; s = applyEffects(s, sc.chapter, scaleFx(sc.perUnit, v), scaleFx(sc.meterPerUnit, v)); rec(v); }
      ...
      const n = sc.prods[s.cond.authority].length; const giveAt = style === "kind" ? 99 : style === "obedient" ? 1 : Math.ceil(Math.random() * (n + 1));
...
for (const style of ["random", "cruel", "kind", "obedient"] as Style[]) {
...
console.log("archetypes reached:", Object.keys(tally).sort().join(", "), "| quota hit rate", (hit / 8000 * 100).toFixed(0) + "%");
